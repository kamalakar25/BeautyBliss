...
router.get('/get/all/service-providers', async (req, res) => {
    try {
        const AllShops = await Shop.find({}, 'name email phone gender dob shopName location designation spAddress createdAt');
...
router.get('/service-providers/pending', async (req, res) => {
    try {
      const pendingProviders = await Shop.find({ approvals: false })
        .select('name email designation location phone shopName spAddress'); 
